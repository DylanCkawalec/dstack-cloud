...
class SPDXAttributor:
            ...
            for line in f:
                ...
                if ">" in line:
                    ...
                    if len(parts) >= 2:
                        ...
                        if "<" in commit_part:
                            commit_email = (
                                commit_part.split("<")[1].split(">")[0].strip()
                            )
                        else:
    ...
    def _is_excluded(self, file_path: Path) -> bool:
        ...
        for pattern in self.exclude_patterns:
            ...
            if pattern.endswith("/"):
                if (
                    rel_path_str.startswith(pattern)
                    or f"/{pattern}" in f"/{rel_path_str}/"
                ):
                    return True
    ...
    def _get_company_contact_email(self, company: str) -> str:
        ...
        return company_contacts.get(
            company, f"contact@{company.lower().replace(' ', '')}.com"
        )
    ...
    def _apply_reuse_annotation(self, file_path: Path, headers: List[str]) -> bool:
        ...
        try:
            ...
            for header in headers:
                cmd.extend(
                    ["--copyright", header]
                )  # Keep the full SPDX-FileCopyrightText: format
...
def main():
    ...
    parser.add_argument(
        "--file", type=str, help="Process a specific file instead of all source files"
    )
    parser.add_argument(
